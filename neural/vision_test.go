...
// TestLightModifier verifies darkness attenuates vision.
func TestLightModifier(t *testing.T) {
	...
	entity := EntityInfo{X: 10, Y: 0, Composition: 0, DigestiveSpec: 0.2, IsFlora: false}

	// Full light
	brightParams := baseParams
	brightParams.LightLevel = 1.0
	var pvBright PolarVision
	pvBright.ScanEntities(brightParams, []EntityInfo{entity})

	// Half light
	dimParams := baseParams
	dimParams.LightLevel = 0.5
	var pvDim PolarVision
	pvDim.ScanEntities(dimParams, []EntityInfo{entity})

	// Darkness
	darkParams := baseParams
	darkParams.LightLevel = 0.0
	var pvDark PolarVision
	pvDark.ScanEntities(darkParams, []EntityInfo{entity})

	// Bright should be 2x dim
	ratio := pvBright.Food[ConeFront] / pvDim.Food[ConeFront]
	if math.Abs(float64(ratio-2.0)) > 0.01 {
		t.Errorf("Bright/Dim ratio = %v, want 2.0", ratio)
	}

	// Dark should be zero
	if pvDark.Food[ConeFront] != 0 {
		t.Errorf("Dark vision = %v, want 0", pvDark.Food[ConeFront])
	}
}
